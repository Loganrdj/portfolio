    ...
    <div className="timeline">
      ...
      {(() => {
        ...
        return sorted.map((exp, i) => {
          ...
          const bracketOffset = 8; // shift brackets when lanes stack
          const connectorLen  = baseConnector + laneIndex * (cardWidth + laneGap);

          // inline styles
          const bracketLeft = side === "left"
            ? `calc(50% - 2% - ${laneIndex * bracketOffset}px)`
            : `calc(50% + 2% + ${laneIndex * bracketOffset}px)`;
          ...
          const cardLeft = side === "left"
            ? `calc(50% - 2% - ${laneIndex * bracketOffset}px - ${connectorLen + cardWidth}px)`
            : `calc(50% + 2% + ${laneIndex * bracketOffset}px + ${connectorLen}px)`;
